...
MAXTEXT_COMMAND="MODEL_IMPL_TYPE=flax_nnx \
  ...
  checkpoint_storage_use_zarr3=False rollout_data_parallelism=2 \
  rollout_tensor_parallelism=8 hbm_utilization_vllm=0.2 \
  batch_size=8 profiler=xplane profiler_steps=2 \
  num_batches=500 \
  base_emb_dim=2880 vocab_size=201088 \
  batch_size=16 \
