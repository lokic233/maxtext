...
class LayerwiseQuantization:
  ...
  def _load_and_quantize_nnx(self) -> None:
    ...
    # We filter out transient runtime state (RngState, Cache, Intermediate, NormStat) so only persistent weights are saved.
    convert_state = (
        nnx.state(convert_model)
        .filter(lambda path, var: not isinstance(var, (nnx.RngState, nnx.Cache, nnx.Intermediate, nnx.NormStat)))
        .to_pure_dict()
    )
