...
class GrainCheckpointableEquivalenceTest(parameterized.TestCase):
  ...
  def test_save_restore_equivalence_single_item(self):
    ...
    # v1 Save
    wrapper = checkpointing.GrainCheckpointable(
        save_args=checkpointing.GrainCheckpointSave(item=iterator_v1)
    )
  ...
  def test_save_restore_equivalence_list_item(self):
    ...
    # v1 Save
    wrapper = checkpointing.GrainCheckpointable(
        save_args=checkpointing.GrainCheckpointSave(item=item_v1)
    )
    ...
    args_v0 = checkpointing.GrainCheckpointRestore(
        item=iterators_restore_v0, process_index=[0, 1], process_count=2
    )
    handler.restore(v0_path, args=args_v0)
